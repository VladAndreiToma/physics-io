...
import ls1 from "../images/ls1.jpeg";   import ls2 from "../images/ls2.jpeg";     import ls3 from "../images/ls3.png";
...
export default function LasersSystems(){
  ...
  - Stimulated vs Spontaneous Emission:  
    ...
    Stimulated emission happens when an incoming photon with energy \\( h\\nu \\) causes an excited electron to drop energy level, releasing a photon of identical energy, phase, and direction.
  
  - Einstein Coefficients:  
    The transition rates between energy levels are governed by:  
    \\( A_{12} \\) (spontaneous emission), \\( B_{12} \\) (stimulated emission), \\( B_{21} \\) (absorption).  
    The net optical gain depends on population inversion and these coefficients.
  
  - Population Inversion:  
    Required for lasing. Population of higher energy level \\( N_2 \\) exceeds lower level \\( N_1 \\):  
    \\( N_2 > N_1 \\)  
  ...
  - Gas Lasers:  
    Examples: HeNe, Argon-ion, CO₂. Excited atoms or ions produce laser emission. CO₂ lasers operate at \\( 10.6 \\, \\mu m \\), useful in cutting/welding.
  ...
  - Transverse Electromagnetic Modes (TEM):  
    Characterize beam spatial profiles: \\( \\mathrm{TEM}_{00} \\) is the Gaussian mode with highest quality and lowest divergence.
  ...
  - Stability Condition:  
    ...
    \\[
      0 \\leq g_1 g_2 \\leq 1
    \\]  
    where \\( g = 1 - \\frac{L}{R} \\) (\\( L \\) = cavity length, \\( R \\) = mirror radius).
  `
    },
    {
  ...
  - Divergence and Beam Waist:  
    Gaussian beams have minimum waist \\( w_0 \\) and diverge over distance \\( z \\).  
    Beam divergence \\( \\theta \\approx \\frac{\\lambda}{\\pi w_0} \\)
  
  - Pulse Duration and Q-Switching:  
    Q-switching stores energy then releases it in a short, intense burst.  
    Pulse energy \\( E = \\frac{P_{avg}}{\\text{repetition rate}} \\).
  ...
  - Power and Energy:  
    ...
    - Pulsed: defined energy per pulse \\( (E = \\int P(t) dt) \\)
  `
    },
    {
  ...
  - Ultrafast Lasers:  
    Generate pulses \\(< 100 \\, \\mathrm{fs} \\) using mode locking. Key in pump-probe experiments, micromachining, time-resolved spectroscopy.
    ...
    return(
        <div className="course-page">
            ...
            {!talkToNewtonium ? 
                ...
                <CoursePictureShowcaseContainer images={[ls1,ls2,ls3]}/>
